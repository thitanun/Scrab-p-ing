...
class testHallyukstar:
    ...
    def data_domain_for_ref(self, session, url):
        with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for div in bs.find_all('h2', {'class' : 'post-title'}):
                a = div.find('a')
            ...
            self.webdata['website']['www.hallyukstar.com'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://www.hallyukstar.com/category/news/']
        n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('div', {'class' : 'entry-content entry clearfix is-expanded'})
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Hallyukstar.json', 'w', encoding="UTF-8") as file:
