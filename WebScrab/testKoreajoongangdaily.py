...
class testKoreajoongangdaily:
    ...
    def data_domain_for_ref(self, session, url):
         with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for div in bs.find_all('div', {'class' : 'mid-article3'}):
                a = div.find('a')
            ...
            self.webdata['website']['koreajoongangdaily.joins.com'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://koreajoongangdaily.joins.com/section/kpop']
        n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('div', {'id' : 'article_body'})
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Koreajoongangdaily.json', 'w', encoding="UTF-8") as file:
