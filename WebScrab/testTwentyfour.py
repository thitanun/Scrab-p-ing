...
class testTwentyfour:
    ...
    #         news_link = []
    #         list_link = []
    #         for h2 in bs.find_all('h2', {'class' : 'post-title'}):
    #             a = h2.find('a')
    ...
    #         self.webdata['website']['kpop.youzab.com'] = list_link


    # def scrap_for_ref(self):
    #     pages_list = [f'https://kpop.youzab.com/page/{i}' for i in range(1, 11)]
    #     n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('div', {'class' : 'td_block_wrap tdb_single_content tdi_84 td-pb-border-top td_block_template_1 td-post-content tagdiv-type'})
    ...
    def data_domain(self, session, url):
        with session.get(url) as response:
            ...
            news_link = []
            for div in bs.find_all('div', {'class' : 'td-module-meta-info'}):
                a = div.find('a')
                news_link.append(a.attrs['href'])
    ...
    def scrap(self):
        pages_list = [f'https://www.twentyfour-news.com/category/kpop/page/{i}' for i in range(1, 6)]
        n = len(pages_list)
        # print('test page', pages_list)
        with ThreadPoolExecutor(max_workers=n) as executor:
            with requests.Session() as session:
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Twentyfour.json', 'w', encoding="UTF-8") as file:
