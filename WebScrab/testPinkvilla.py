...
class testPinkvilla:
    ...
    def data_domain_for_ref(self, session, url):
        with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for div in bs.find_all('div', {'class' : 'views-row views-row-38 views-row-even'}):
                a = div.find('a')
            ...
            self.webdata['website']['www.pinkvilla.com'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://www.pinkvilla.com/entertainment/korean/news/page/{i}' for i in range(1, 11)]
        n = len(pages_list)
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Pinkvilla.json', 'w', encoding="UTF-8") as file:
