...
class testTheguardian:
    ...
    def data_domain_for_ref(self, session, url):
       with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for div in bs.find_all('div', {'class' : 'fc-item__container'}):
                a = div.find('a')
            ...
            self.webdata['website']['www.theguardian.com'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://www.theguardian.com/music/k-pop?page={i}' for i in range(1,6)]
        n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('div', {'class' : 'dcr-1jw1u7l'})
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Theguardian.json', 'w', encoding="UTF-8") as file:
