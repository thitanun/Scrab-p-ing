...
class testKpopmap:
    ...
    def data_domain_for_ref(self, session, url):
        with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for div in bs.find_all('div', {'class' : 'post-content'}):
                a = div.find('a')
            ...
            self.webdata['website']['www.kpopmap.com'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://www.kpopmap.com/page/{i}' for i in range(1, 11)]
        n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('div', {'class' : 'article'})
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Kpopmap.json', 'w', encoding="UTF-8") as file:
