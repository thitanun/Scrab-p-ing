...
class testHungerplus:
    ...
    def data_domain_for_ref(self, session, url):
        with session.get(url) as response:
            ...
            self.webdata['website']['hungerplus.org'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://hungerplus.org/archives/tag/k-pop/page/{i}' for i in range(1, 11)]
        n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('main', {'class' : 'site-main'})
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Hungerplus.json', 'w', encoding="UTF-8") as file:
