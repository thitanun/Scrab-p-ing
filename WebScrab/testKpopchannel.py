...
class testKpopchannel:
    ...
    def data_domain_for_ref(self, session, url):
        with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for div in bs.find_all('div', {'class' : 'wrapper has-thumb'}):
                a = div.find('a')
            ...
            self.webdata['website']['www.kpopchannel.tv'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://www.kpopchannel.tv/news/page/{i}' for i in range(1, 11)]
        n = len(pages_list)
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Kpopchannel.json', 'w', encoding="UTF-8") as file:
