...
class testMetro:
    ...
    def data_domain_for_ref(self, session, url):
         with session.get(url) as response:
            ...
            news_link = []
            list_link = []
            for h in bs.find_all('h3', {'class' : 'metro__post__title'}):
                a = h.find('a')
            ...
            self.webdata['website']['metro.co.uk'] = list_link


    def scrap_for_ref(self):
        pages_list = [f'https://metro.co.uk/tag/k-pop/page/{i}' for i in range(1, 11)]
        n = len(pages_list)
    ...
    def data_in_link(self, session, url):
        ...
        with session.get(url) as response:
            ...
            self.content = ''
            section = bs.find('div', {'class' : 'article-body'})
    ...
    def save_to_json(self):
        with open(f'./WebScrab/Metro.json', 'w', encoding="UTF-8") as file:
